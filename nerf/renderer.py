...
from Volumetric.VoxelRender import VoxelRender

# import torch_scatter
TORCH_SCATTER = None  # lazy import
from Volumetric import VoxelRender
...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        if self.individual_dim > 0:
            self.individual_codes = nn.Parameter(torch.VoxelRenderrandn(self.individual_num, self.individual_dim) * 0.1)
        else:
        ...
        self.mean_density = 0
        self.iter_density = 0
        print('1 ：density_grid:', torch.min(self.density_grid))
        ...
        if self.opt.stage == 1:
            ...
            for cas in range(self.cascade):
                ...
                _updated_mesh_path = os.path.join(self.opt.workspace, 'mesh_stage0',
                                                  f'mesh_{cas}_updated.ply') if self.opt.mesh == '' else self.opt.mesh
                if os.path.exists(_updated_mesh_path) and self.opt.ckpt != 'scratch':
                    ...
                    mesh = trimesh.load(os.path.join(self.opt.workspace, 'mesh_stage0', f'mesh_{cas}.ply'),
    ...
    def get_params(self, lr):
        ...
        if self.opt.trainable_density_grid:
            ...
            print('2 ：density_grid:', torch.min(self.density_grid))
    ...
    def refine_and_decimate(self):
        ...
        if self.bound <= 1:
            ...
            v, f = decimate_and_refine_mesh(v, f, mask, decimate_ratio=self.opt.refine_decimate_ratio,
    ...
    def export_stage0(self, save_path, resolution=None, decimate_target=1e5, dataset=None, S=128):
        ...
        if resolution == self.grid_size:
            ...
            print('3 ：density_grid:', torch.min(self.density_grid))
        else:
            # query
            ...
            if not self.opt.sdf:
                ...
                print('4 ：density_grid:', torch.min(self.density_grid))
        ...
        vertices, triangles = clean_mesh(vertices, triangles, min_f=self.opt.clean_min_f, min_d=self.opt.clean_min_d,
        ...
        if self.bound > 1:
            ...
            for cas in range(1, self.cascade):
                ...
                print('5 ：density_grid:', torch.min(self.density_grid))
    ...
    def render(self, rays_o, rays_d, index=None, dt_gamma=0, bg_color=None, perturb=False, max_steps=1024,
               T_thresh=1e-4, cam_near_far=None, shading='full', VoxelModel=None, **kwargs):
        # rays_o, rays_d: [N, 3]
        ...
        N = rays_o.shape[0]
        device = rays_o.device  # CUDA

        # pre-calculate near far
        # print('self.aabb_train', self.aabb_train, 'self.aabb_infer', self.aabb_infer)
        nears, fars = raymarching.near_far_from_aabb(rays_o, rays_d,
                                                     self.aabb_train if self.training else self.aabb_infer,
                                                     self.min_near)  # 0.05 计算获得在bound内的xyz轴上的最小的near与far
        ...
        results = {}
        # print('density_bitfield',self.density_bitfield.shape)
        if self.training:
            xyzs, dirs, ts, rays = raymarching.march_rays_train(rays_o, rays_d, self.real_bound, self.opt.contract,
                                                                self.density_bitfield, self.cascade, self.grid_size,
                                                                nears, fars, perturb, dt_gamma, max_steps)
            ...
            weights_sum = torch.zeros(N, dtype=dtype, device=device)  # =权重和
            ...
            while step < max_steps:
                ...
                # decide compact_steps 决定在一起的步数
                n_step = max(min(N // n_alive, 8), 1)
                # rays_t = nears.clone()  # [N] contract = False cascade = 5 False 128
                # print("real_bound", self.real_bound)
                ...
                dirs = safe_normalize(dirs)
                with torch.cuda.amp.autocast(enabled=self.opt.fp16):
                    # print(f'[xyzs] min={xyzs.min().item():.4f}, max={xyzs.max().item():.4f}')

                    sigmas, rgbs, speculars = self(xyzs, dirs, ind_code, shading)

                    # if VoxelModel is not None:
                    #     sigmas = voxel_sigmas
                    #     rgbs = voxel_rgb

                    # print(
                    #     f'[sigmas] min={sigmas.min().item():.4f}, max={sigmas.max().item():.4f},'
                    #     f'[rgbs] max = {rgbs.max().item():.4f},min = {rgbs.min().item():.4f} , '
                    #     f'[speculars] max = {speculars.max().item():.4f},min = {speculars.min().item():.4f} ')
                ...
                if VoxelModel is None:
                    raymarching.composite_rays(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, ts, weights_sum,
                                               depth,
                                               image, T_thresh, self.opt.sdf)  # 如何做sigms的转换的呢？
                else:
                    voxel_sigmas, voxel_rgb = VoxelModel.Render(xyzs)
                    print(f'[sigmas] min={voxel_sigmas.min().item():.4f}, max={voxel_sigmas.max().item():.4f}')
                    print(f'[voxel_rgb] min={voxel_rgb.min().item():.4f}, max={voxel_rgb.max().item():.4f}')
                    raymarching.fusion_composite_rays(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, voxel_sigmas,
                                                      voxel_rgb, ts, weights_sum, depth, image, T_thresh, self.opt.sdf)
    ...
    # phase 2
    def render_stage1(self, rays_o, rays_d, mvp, h0, w0, index=None, bg_color=None, shading='full', VoxelModel=None,
                      **kwargs):
    ...
    def mark_untrained_grid(self, dataset, S=64):
        ...
        poses = dataset.poses  # [B, 4, 4] 姿态
        intrinsics = dataset.intrinsics  # [4] or [B/1, 4] 本征
                ...
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        while head < B:
                            ...
                            min_near = self.opt.min_near if cam_near_far is None else cam_near_far[head:tail,
                                                                                      0].unsqueeze(1)
        ...
        self.density_grid[((mask_cam == 0) | (mask_aabb == 0))] = -1  # 不去重建的地方

        print(
            f'[mark untrained grid] {((mask_cam == 0) | (mask_aabb == 0)).sum()} from {self.grid_size ** 3 * self.cascade}')

    def update_extra_state(self, decay=0.95, S=128):  # 16步更新一次的密度网格
            ...
            for xs in X:
                for ys in Y:
                    for zs in Z:
                        ...
                        xx, yy, zz = custom_meshgrid(xs, ys, zs)
                        coords = torch.cat([xx.reshape(-1, 1), yy.reshape(-1, 1), zz.reshape(-1, 1)],
                                           dim=-1)  # [N, 3], in [0, 128) 做出全部的坐标
                        indices = raymarching.morton3D(coords).long()  # [N]
                        xyzs = 2 * coords.float() / (self.grid_size - 1) - 1  # [N, 3] in [-1, 1] 映射到-1 1

                        # cascading
                        for cas in range(self.cascade):
                            bound = min(2 ** cas, self.bound)  # 选择aabb的大小
                            half_grid_size = bound / self.grid_size  # 计算每个分辨率的尺寸
                            # scale to current cascade's resolution
                            cas_xyzs = xyzs * (bound - half_grid_size)  # 扩大回来 回到当前的分辨率 更新即可
                            # add noise in [-hgs, hgs]
                            cas_xyzs += (torch.rand_like(cas_xyzs) * 2 - 1) * half_grid_size  # 加入噪声
                            # query density
                            with torch.cuda.amp.autocast(enabled=self.opt.fp16):
        ...
        if not self.opt.trainable_density_grid:  # 可训练密度网格
            ...
            if self.opt.lambda_density > 0:
                for cas in range(1, self.cascade):
                    loss_density = loss_density + (2 ** (cas - 1)) * self.opt.lambda_density * self.density_grid[cas][
                        valid_mask[cas]].mean()

        self.mean_density = torch.mean(
            self.density_grid.clamp(min=0)).item()  # -1 regions are viewed as 0 density. 求平均值 压缩到0 ~ 无穷
        ...
        # print(f'[density grid] min={self.density_grid.min().item():.4f}, max={self.density_grid.max().item():.4f},'
        #       f'tmp_max = {tmp_grid.max().item():.4f},tmp_min = {tmp_grid.min().item():.4f} mean={self.mean_density:.4f}, '
        #       f'occ_rate={(self.density_grid > density_thresh).sum() / (128 ** 3 * self.cascade):.3f}')
        if not self.opt.trainable_density_grid:
            return None
        else:
            return loss + loss_density

    def update_extra_model_state(self, VoxelModel, S=128):
        tmp_grid = - torch.ones_like(self.density_grid)
            ...
            for ys in Y:
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        cas_xyzs += (torch.rand_like(cas_xyzs) * 2 - 1) * half_grid_size  # 加入噪声
                        # 开始置换吧
                        rays_densities, rays_features = VoxelModel.Render(cas_xyzs)
                        sigmas = rays_densities
                        tmp_grid[cas, indices] = sigmas * 100.0

        valid_mask = (tmp_grid > 0)

        self.density_grid[valid_mask] = torch.maximum(self.density_grid[valid_mask], tmp_grid[valid_mask])
