...
class Trainer(object):
    ...
    def test_step(self, data, bg_color=None, perturb=False, shading='full', VoxelModel=None):
        ...
        if self.opt.stage > 0:
            ...
            outputs = self.model.render(rays_o, rays_d, index=index, staged=True, bg_color=bg_color, perturb=perturb,
                                        cam_near_far=cam_near_far, shading=shading,VoxelModel = VoxelModel, **vars(self.opt))
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        print('3 :trainer ：density_grid:', torch.min(self.model.density_grid))
        if self.use_tensorboardX and self.local_rank == 0:
            self.writer = tensorboardX.SummaryWriter(os.path.join(self.workspace, "run", self.name))
        print('4 :trainer ：density_grid:', torch.min(self.model.density_grid))
        # mark untrained region (i.e., not covered by any camera from the training dataset)
        if self.opt.mark_untrained:
            self.model.mark_untrained_grid(train_loader._data)
        print('5 :trainer ：density_grid:', torch.min(self.model.density_grid))
        start_t = time.time()
        print('6 :trainer ：density_grid:', torch.min(self.model.density_grid))
    ...
    def test_gui(self, pose, intrinsics, mvp, W, H, bg_color=None, spp=1, downscale=1, shading='full', VoxelModel=None):
        ...
        with torch.no_grad():
            ...
            preds, preds_depth = self.test_step(data, bg_color=bg_color, perturb=False if spp == 1 else spp,
                                                shading=shading, VoxelModel=VoxelModel)
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            # update grid every 16 steps 每16步更新一次密度网格
            if self.model.cuda_ray and self.global_step % self.opt.update_extra_interval == 0:
                loss_grid = self.model.update_extra_state()
            else:
                loss_grid = None
