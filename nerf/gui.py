...
from Volumetric.Volumetric import Mesh2Voxel
...
class NeRFGUI:
    ...
    def TrainVoxel(self, Transformation=(0, 0, 0), Rotation=(0, 0, 0), Scale=(1.0, 1.0, 1.0)):
        mesh2Voxel = Mesh2Voxel(num_view=70, IsShow=False)
        mesh2Voxel.train(15, 500)
        mesh2Voxel.SetTrainModel(Transformation, Rotation, Scale)

        self.VoxelModel = VoxelRender(mesh2Voxel.ResultVoxel)

        self.trainer.model.update_extra_model_state(self.VoxelModel)
    ...
    def test_step(self):
        ...
        if self.need_update or self.spp < self.opt.max_spp:
            ...
            outputs = self.trainer.test_gui(self.cam.pose, self.cam.intrinsics, mvp, self.W, self.H, self.bg_color,
                                            self.spp, self.downscale, self.shading, self.VoxelModel)
    ...
    def register_dpg(self):
        ...
        with dpg.window(label="Control", tag="_control_window", width=400, height=300):
            ...
            with dpg.collapsing_header(label="Options", default_open=True):
                ...
                dpg.add_combo(('full', 'diffuse', 'specular'), label='shading', default_value=self.shading,
