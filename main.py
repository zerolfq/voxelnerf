...
if __name__ == '__main__':
    ...
    parser.add_argument('--background', type=str, default='random', choices=['white', 'random'],
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            # torch.deg2rad(torch.Tensor([270, 0, 0]))
            gui.TrainVoxel((0, 0, 0),torch.deg2rad(torch.Tensor([270, 0, 0])),(0.5,0.5,0.5))
            gui.render()
        ...
        scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 0.01 + 0.99 * (
                iter / 500) if iter <= 500 else 0.1 ** ((iter - 500) / (opt.iters - 500)))

        trainer = Trainer('ngp', opt, model, device=device, workspace=opt.workspace, optimizer=optimizer,
                          criterion=criterion, ema_decay=0.95 if opt.stage == 0 else None, fp16=opt.fp16,
        ...
        print('1 :trainer ：density_grid:', torch.min(trainer.model.density_grid))
        if opt.gui:
            ...
            print('2 :trainer ：density_grid:', torch.min(trainer.model.density_grid))
            trainer.metrics = [PSNRMeter(), ]
            trainer.train(train_loader, valid_loader, max_epoch)  # 开始训练

            # last validation
            trainer.metrics = [PSNRMeter(), SSIMMeter(), LPIPSMeter(device=device)]
            trainer.evaluate(valid_loader)
            print('3 :trainer ：density_grid:', torch.min(trainer.model.density_grid))
            # also test
            test_loader = NeRFDataset(opt, device=device, type='test').dataloader()
