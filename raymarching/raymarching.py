...
class _near_far_from_aabb(Function):
    ...
    def forward(ctx, rays_o, rays_d, aabb, min_near=0.2):
        ...
        N = rays_o.shape[0]  # num rays 光线的数量
...
class _morton3D(Function):
    ...
    def forward(ctx, coords):
        ...
        N = coords.shape[0]  # 128 * 128 * 128

        indices = torch.empty(N, dtype=torch.int32, device=coords.device)  # 128 * 128 * 128
...
class _march_rays_train(Function):
    ...
    def forward(ctx, rays_o, rays_d, bound, contract, density_bitfield, C, H, nears, fars, perturb=False, dt_gamma=0,
        ...
        _backend.march_rays_train(rays_o, rays_d, density_bitfield, bound, contract, dt_gamma, max_steps, N, C, H,
...
class _composite_rays_train(Function):
    ...
    def forward(ctx, sigmas, rgbs, ts, rays, T_thresh=1e-4, alpha_mode=False):
        ...
        _backend.composite_rays_train_forward(sigmas, rgbs, ts, rays, M, N, T_thresh, alpha_mode, weights, weights_sum,
                                              depth, image)
    ...
    def backward(ctx, grad_weights, grad_weights_sum, grad_depth, grad_image):
        ...
        _backend.composite_rays_train_backward(grad_weights, grad_weights_sum, grad_depth, grad_image, sigmas, rgbs, ts,
                                               rays, weights_sum, depth, image, M, N, T_thresh, alpha_mode, grad_sigmas,
                                               grad_rgbs)
...
class _march_rays(Function):
    ...
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, bound, contract, density_bitfield, C, H, near,
        ...
        M = n_alive * n_step  # 多少个光线

        xyzs = torch.zeros(M, 3, dtype=rays_o.dtype, device=rays_o.device)
        dirs = torch.zeros(M, 3, dtype=rays_o.dtype, device=rays_o.device)
        ts = torch.zeros(M, 2, dtype=rays_o.dtype, device=rays_o.device)  # 2 vals, one for rgb, one for depth 深度与颜色
        ...
        _backend.march_rays(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, bound, contract, dt_gamma, max_steps,
                            C, H, density_bitfield, near, far, xyzs, dirs, ts, noises)
...
class _composite_rays(Function):
    ...
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, ts, weights_sum, depth, image, T_thresh=1e-2,
        ...
        sigmas = sigmas.float().contiguous()
        rgbs = rgbs.float().contiguous()
        _backend.composite_rays(n_alive, n_step, T_thresh, alpha_mode, rays_alive, rays_t, sigmas, rgbs, ts,
...
class _fusion_composite_rays(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)  # need to cast sigmas & rgbs to float
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, voxel_sigmas, voxel_rgb, ts, weights_sum, depth,
                image, T_thresh=1e-2,
                alpha_mode=False):
        ''' composite rays' rgbs, according to the ray marching formula. (for inference)
        Args:
        ...
        sigmas = sigmas.float().contiguous()
        rgbs = rgbs.float().contiguous()
        voxel_sigmas = voxel_sigmas.contiguous()
        voxel_rgb = voxel_rgb.contiguous()

        _backend.fusion_composite_rays(n_alive, n_step, T_thresh, alpha_mode, rays_alive, rays_t, sigmas, rgbs,
                                       voxel_sigmas, voxel_rgb, ts,
                                       weights_sum, depth, image)
        return tuple()


fusion_composite_rays = _fusion_composite_rays.apply
