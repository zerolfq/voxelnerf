...
inline __device__ int mip_from_pos(const float x, const float y, const float z, const float max_cascade) {
    const float mx = fmaxf(fabsf(x), fmaxf(fabsf(y), fabsf(z))); // xyz 的最大值
    int exponent;
    frexpf(mx, &exponent); // [0, 0.5) --> -1, [0.5, 1) --> 0, [1, 2) --> 1, [2, 4) --> 2, ... 获取浮点数的指数和尾数
...
__global__ void kernel_near_far_from_aabb(
    ...
    const float rdx = 1 / dx, rdy = 1 / dy, rdz = 1 / dz; //为什么取反向的值呢

    // get near far (assume cube scene)
    float near = (aabb[0] - ox) * rdx; // 计算near的 x轴方向上的距离
    float far = (aabb[3] - ox) * rdx; // 计算far的 y轴方向上的距离
...
__global__ void kernel_march_rays(
    ...
    const float near = nears[index], far = fars[index]; // 远近平面

    const float dt_min = 2 * SQRT3() / max_steps;  // 1024
    const float dt_max = 2 * SQRT3() * bound / H;
    // const float dt_max = 1e10f;

    // march for n_step steps, record points 前进n步
    float t = rays_t[index];
    t += clamp(t * dt_gamma, dt_min, dt_max) * noise; // 增加噪声
    uint32_t step = 0;
...
__global__ void kernel_composite_rays(
    ...
    while (step < n_step) {
        ...
        const float alpha = alpha_mode ? (float)sigmas[0] : (1.0f - __expf(- sigmas[0] * ts[1]));  // sigmas 乘上 变化率 归一化
        ...
        */
        const float T = 1 - weight_sum; //  计算权重
        const float weight = alpha * T;
...
template <typename scalar_t>
__global__ void kernel_fusion_composite_rays(
    const uint32_t n_alive,
    const uint32_t n_step,
    const float T_thresh,
    const bool alpha_mode,
    ...
    const scalar_t* __restrict__ sigmas,
    const scalar_t* __restrict__ rgbs,
    const scalar_t* __restrict__ voxel_sigmas,
    const scalar_t* __restrict__ voxel_rgb,
    const scalar_t* __restrict__ ts,
    scalar_t* weights_sum, scalar_t* depth, scalar_t* image
) {
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    sigmas += n * n_step;
    // =========================================
    voxel_sigmas += n * n_step;
    voxel_rgb += n * n_step * 3;
    // ==========================================
    rgbs += n * n_step * 3;
    ts += n * n_step * 2;
    ...
    while (step < n_step) {
        ...
        const float alpha = alpha_mode ? (float)sigmas[0] : (1.0f - __expf(- sigmas[0] * ts[1]));  // sigmas 乘上 变化率 归一化
        //  体素的alpha值
        const float voxel_alpha = voxel_sigmas[0];
        /*
        T_0 = 1; T_i = \prod_{j=0}^{i-1} (1 - alpha_j)
        w_i = alpha_i * T_i
        -->
        ...
        */
        const float T = 1 - weight_sum; //  计算权重
        const float weight = max(alpha,voxel_alpha) * T; // 加入权重
        weight_sum += weight; // 累加权重

        float fusion_r = voxel_alpha > alpha ? voxel_rgb[0] : rgbs[0],
        fusion_g = voxel_alpha > alpha ? voxel_rgb[1] : rgbs[1],
        fusion_b = voxel_alpha > alpha ? voxel_rgb[2] : rgbs[2];
...
void fusion_composite_rays(const uint32_t n_alive, const uint32_t n_step, const float T_thresh, const bool alpha_mode, at::Tensor rays_alive, at::Tensor rays_t, at::Tensor sigmas, at::Tensor rgbs,at::Tensor voxel_sigmas ,at::Tensor voxel_rgb , at::Tensor ts, at::Tensor weights, at::Tensor depth, at::Tensor image) {
    static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    image.scalar_type(), "fusion_composite_rays", ([&] {
        kernel_fusion_composite_rays<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, T_thresh, alpha_mode, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(),voxel_sigmas.data_ptr<scalar_t>(),voxel_rgb.data_ptr<scalar_t>(), ts.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>());
    }));
}